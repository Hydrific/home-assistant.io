...
{% note %}
In order to report high and unusual flows, Droplet must be connected to Hydrific servers and have firmware v1.4.0 or higher.
{% endnote %}
...
**A**: It may take up to a minute for these sensors to update after initial setup. If they are still showing up as unknown, make sure your Droplet is connected to the server, and that its firmware is v1.4.0 or higher. High flow and unusual flow statuses require Hydrific server connectivity.
